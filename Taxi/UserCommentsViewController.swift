...
class UserCommentsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var postsRef:DatabaseReference {
        return Database.database().reference().child("private_posts").child(Common.instance.getUserId()).child("Comments")
    ...
    func observePosts() {
        let commentsRef = Database.database().reference().child("posts").child(self.postid!).child("")
        ...
        commentsRef.observe(.value, with: { snapshot in
            ...
            for child in snapshot.children {
                if let childSnapshot = child as? DataSnapshot,
                    ...
                    let travel_id_var = dict["travel_id"]as?Double,
//                    let privacy = dict["privacy"]as?String,
                    let timestamp = dict["timestamp"] as? Double {
                    
                    let userProfile = UserProfile(uid: uid, username: username, photoURL: url)
                    let post = Post(id: childSnapshot.key, author: userProfile, text: text, timestamp:timestamp,type: type, travel_id: Int(travel_id_var))
    ...
    func handleCommentButton() {
        if CommentTextField.text!.count > 0
        {
            ...
            let postRef = Database.database().reference().child("posts").child(self.postid!).child("Comments").childByAutoId()
            let postObject = [
                "author": [
                    "uid": userProfile.uid,
                ...
                "text": CommentTextField.text!,//"comment1 test",
